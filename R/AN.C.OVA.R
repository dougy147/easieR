AN.C.OVA <-
  function(option=T, longdata=NULL, inter=NULL, intra=NULL, VD=NULL, cov=NULL, desires=c("Donnees completes","Identification des outliers", "Data without influencing value"),
    ...
    # cov : covariables
    # desires : vecteur avec une plusieurs des possibilites suivantes : c("Donnees completes","Identification des outliers", "Data without influencing value"). Si desires est specifie
    # desires2 :
    ...
    large.long<-function(data, VIR){
      ...
      if(length(VIR)>3) N.facteurs <- dlgInput("How many factors can be repeated?", 1)$res else N.facteurs<-"1"
      while(length(N.facteurs)=="0"){writeLines("you must specify the number of factors able to repeat")
        dlgMessage("You did not specify the number of factors able to repeat, do you want to quit?", "yesno")$res->quitte
        if(quitte=="yes") return(NULL) else  N.facteurs <- dlgInput("How many factors can be repeated?", 1)$res }
      ...
      if(N.facteurs==1){list()->intra
        ...
        while(prod(N.modalites2)!=length(VIR)){list()->intra
          ...
          writeLines("the product of the modalities of each of the variables must correspond to the number of selected columns.")
      ...
      line <- readline()
      dlgMessage("Is the long format structure of your data correct?", "yesno")$res->suppression
    ...
    options.aov<-function(inter, intra){
      ...
      if(!is.null(cov)) {
        ...
        if((exists("inter") && (length(inter)==1 & is.null(intra))) | (!is.null(intra) && (length(intra)==1 & is.null(inter)))) {
          Resultats$desires2<- dlgList(c("Modele parametrique", "Modele non parametrique", "Modele lineaire mixte",
                                         "Robust statistics - can take time"),
                                       preselect=c("Modele parametrique", "Modele lineaire mixte", "Modele non parametrique", "Robust statistics - can take time"),
                                       multiple = TRUE, title="Quelle(s) analyses voulez-vous  ?")$res
        } else {
          if((exists("inter") && (length(inter)==1 & !is.null(intra) && length(intra)==1)) || (exists("inter") && (length(inter)<4 & is.null(intra)))) {
            Resultats$desires2<- dlgList(c("Modele parametrique","Modele lineaire mixte", "Robust statistics - can take time"),
                                         preselect=c("Modele parametrique", "Modele lineaire mixte", "Robust statistics - can take time"),
                                         multiple = TRUE, title="Quelle(s) analyses voulez-vous  ?")$res
          } else {
      ...
      if(any(Resultats$desires2 %in% c("Modele parametrique","Modele lineaire mixte"))){
        ...
        Resultats$desires<- dlgList(c("Donnees completes","Identification des outliers", "Data without influencing value"),
                                    preselect=c("Donnees completes","Identification des outliers", "Data without influencing value"),
      ...
      writeLines("Do you want to save the results of the analysis?")
            ...
            type.cont2<- dlgList(c("orthogonaux", "orthogonaux inverses", "polynomiaux","comparaison a une ligne de base", "specifier les contrastes"),
                                 preselect=c("orthogonaux"), multiple = FALSE, title=paste("What contrasts for the variable",names(longdata[interintra])[i],"?"))$res} else {
                                   type.cont2<- dlgList(c("orthogonaux", "orthogonaux inverses", "polynomiaux","comparaison a une ligne de base",
                                                          "specifier les contrastes"),preselect=c("orthogonaux"), multiple = FALSE, title=paste("What contrasts for the variable",names(longdata[interintra])[i],"?"))$res
                                 }
      ...
      if(any(type.v=="Mesure repetee")) {
        ...
        while(any(VIR=="autres")){
          ...
          if(all(sapply(data[,VIR], class)=="factor")){
            data->longdata
            VIR->intra
            writeLines("What is the variable identifying the participants?")
            ...
            if(nlevels(longdata$IDeasy)*prod(N.modalites2)!=length(longdata[,1])) {
              okCancelBox("Each participant must appear once and only once for each combination of modalities")
              return(AN.C.OVA())}
          }else{
            format<-"court"
            if(length(VIR)==1) {
    ...
    if(plan=="Groupes independants" || format=="long") {
      writeLines("Please choose the dependent variable.")
      ...
      if(!is.null(intra)) {
        if( min(table(longdata$IDeasy))!=  max(table(longdata$IDeasy)))  msgBox("Some participants have missing values on repeated measures factors. They will be removed from the analyzes")
    ...
    if(any(type.v=="Covariables")) {
      ...
      writeLines("Please choose the covariate (s)")
    ...
    if(length(unique(longdata[,VD]))<3) return("The dependent variable has less than three different values. Check your data or the analysis you are trying to perform is irrelevant.")
    ...
    anova2<-function(VD=NULL, inter=NULL, intra=NULL, longdata,  type.cont,p.adjust, SumS, desires2, cov=NULL, contrastes=NULL)   {
      ...
      for(i in 1:length(c(inter, unlist(intra)))){
        ...
        for(j in 1:ncol(facteurs)){
          ...
          if(nrow( facteurs) ==1) paste("Descriptive statistics of the variable", facteurs[,j])->nsd else {
            paste("Descriptive statistics of the interaction between", facteurs[1,j])->nsd
      ...
      if(any(desires2=="Modele parametrique") | any(desires2=="Modele lineaire mixte")){
        ...
        if(!is.null(cov) & !is.null(inter)){
          ...
          for(i in 1:length(cov)){
            ...
            aov.cov->Resultats$"Ancova application conditions"[[paste0("Test for the absence of difference between groups on ", cov[i])]]
            if(i==1) {paste(cov[1],"*")->cov2} else {paste0(cov2, cov[i],"*")->cov2}
          }
          aov(as.formula(paste0(VD, "~", cov2,pred.ind)), data=longdata)->aov.cov
          Anova(aov.cov, type="III")->aov.cov
          names(aov.cov)<-c("SC", "ddl", "F", "valeur.p")
          aov.cov-> aov.cov->Resultats$"Ancova application conditions"$"Test for homogeneity of slopes between groups on the dependent variable"
        ...
        if(any(desires2=="Modele parametrique")){
          ...
          if(length(inter)==1 & is.null(intra) & is.null(cov)) {oneway.test(as.formula(paste(VD,"~", inter)),data=longdata)->Welch
            ...
            Welch->Resultats$"Anova with Welch's correction for heterogeneous variances"
          }
        }
        ...
        if(type.cont=="a priori" | any(desires2== "Modele lineaire mixte")){
          ...
          if(class(modele.lme1)=="lme"){
            ...
            if(type.cont=="a priori"){
              contrastes->Resultats$"Contrastes a priori"$"Matrix of variable coefficients"
          ...
          if(!is.null(intra) & is.null(inter) & is.null(cov) & type.cont=="a priori"){
            ...
            Table.contrastes->Resultats$"Table of contrasts imitating commercial software"
        ...
        if(!is.null(inter)){
          ...
          if(!is.null(contrastes) && any(rowSums((contrastes[[1]]!=0))==0)) {kruskalmc( as.formula(paste0(VD, "~",inter[1])),
                                                                                        data=longdata, cont='two-tailed')->Resultats$"Analyse non parametrique"$"Test de Kruskal-Wallis - Comparaison a une ligne de base"} else{
                                                                                          kruskalmc( as.formula( paste0(VD, "~",inter[1])), data=longdata)->Resultats$"Analyse non parametrique"$"Kruskal-Wallis test - Two by two comparison"
          ...
          friedmanmc(longdata[,VD], longdata[,intra[[1]]], longdata$IDeasy)->Resultats$"Comparison 2 to 2 for Friedman's ANOVA"
        }
      }

      if(any(desires2=="Robust statistics - can take time")){
        if(length(inter)==1 & is.null(intra)){
          ...
          if(class(AR1)!="try-error"){
            ...
            AR1->Resultats$"Anova based on truncated averages"$"Analyse principale"
            "The probabilities and CIs are estimated on the basis of a bootsrap. The CI is corrected for multiple comparison, unlike the reported probability.->Resultats$"Anova based on truncated averages"$"Information"
            ...
            if(class(cont)!= "try-error") {data.frame(cont$psihat[,2],cont$test[,4],cont$test[,5],cont$test[,2],cont$test[,3],cont2$psihat[,4],cont2$psihat[,5],cont2$psihat[,6])->cont
              ...
              cont->Resultats$"Anova based on truncated averages"$"Contrastes"}
            if(class(cont2)!="try-error") cont2[3]->Resultats$"Anova based on truncated averages"$"Coefficients des contrastes"

          }else{
            "Desole, nous n'avons pas pu calcule l'anova sur les moyennes tronquees."->Resultats$"Anova based on truncated averages"
          }
        ...
        if(length(intra)==1 & is.null(inter)){
          ...
          if(class(ANOVA.tr)!="try-error"){
            ...
            if((nlevels(longdata[,intra[[1]]]))>2) {rmmcp(longdata[,VD],longdata[, intra[[1]]],longdata$IDeasy)->comp
              ...
              comp->Resultats$"Statistiques robustes"$"Comparaisons 2 a 2 sur moyennes tronquees a 20%"}else Resultats$"Statistiques robustes"<-"Sorry, we couldn't calculate the robust anova.
          }
          ...
          }else Resultats$"Statistiques robustes"<-"Sorry, we couldn't calculate the robust anova.
          ...
          if((nlevels(longdata[,intra[[1]]]))>2) {
            ...
            if(class(comp)!="try-error") {paste0("pairdepb(y = longdata$", VD, ", groups = longdata$", intra[[1]],", blocks = longdata$IDeasy)" )->comp$call
              comp->Resultats$"Statistiques robustes"$"Comparaisons 2 a 2 sur les moyennes tronquees a 20% avec bootsrap"}else Resultats$"Statistiques robustes"<-"Sorry, we couldn't calculate the robust anova.
          }
        }
        ...
        if(length(inter)==1 & length(intra)==1){
          ...
          if(class(tronquees)!="try-error"){
            ...
            MoM->Resultats$"Anova on the modified Huber localization estimator"
          }else Resultats$"Statistiques robustes"<-"Sorry, we couldn't calculate the robust anova.
    ...
    if(any(desires=="Identification des outliers")|any(desires=="Data without influencing value")) {
      if(is.null(longdata$residu)) {"The analysis could not be completed"->Resultats$"Arret premature de l'analyse"
        return(Resultats)}
      ...
      if(any(desires=="Data without influencing value")){
        if(!is.null(influentes$"observations influentes"$IDeasy)){
          ...
          anova2(VD=VD, inter=inter, intra=intra, longdata=nettoyees, type.cont=type.cont, SumS=SumS,
                 desires2=desires2, cov=cov,p.adjust=p.adjust, contrastes=contrastes)->Resultats$"Data without influencing value"
          aov.plus.in->aov.plus.list$"Data without influencing value"
        }
        if(all(desires!="Donnees completes"))  complet->Resultats$"Data without influencing value"
