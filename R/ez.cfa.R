ez.cfa <-
  function(model=NULL, X=NULL, data=NULL,ord=NULL, outlier="Complete data",imp="rm", output="default", info=T, sauvegarde=F, mimic=NULL, fixed.x="default", missing="default",information="default", zero.keep.margins="default",zero.add=c(0.5,0),
           ...
           group.equal=c("loadings", "intercepts", "means", "thresholds", "regressions", "residueals", "residueal.covariances", "lv.variances" , "lv.covariances")){
    # model : lavaan model if X is null
    # data : dataframe
    # X : character. names of the variables if model is null
    ...
    options (warn=-1)
    Lav.model<-function(X=NULL, model=NULL, LV=NULL, info=T){
      # X : character. Names pf tje manifest variables
      # LV : character. Vector of latent variable names. 
      # model : lavaan model
      if(!is.null(model)){
        semPlot.model<-try(semPlotModel_lavaanModel(model))
        if(class(semPlot.model)=="try-error"){
          msgBox("The model appears to be incorrect and could not be created.")
          return(NULL)
        }
        semPaths(semPlot.model, edge.label.cex = 0.65,edge.color="black", exoVar = FALSE,exoCov =T, cex=0.5)
        cat ("Appuyez [Betweene] pour continuer")
        line <- readline()
        dlgMessage("Is your model correct?", "yesno")$res->suppression
        if(suppression=="no") return( Lav.model(X=X, model=NULL, LV=NULL, info=T)) 
        return(model)
      }
      ...
      if(is.null(LV) && length(X)>3) {
        ...
        for(i in 1:nF){
          if(is.null(LV[i]))  {dlgInput(paste("Name of the latent variable",i,  "?"), paste("Factor",i, sep="."))$res->noms
            if(length(noms)==0) return(Lav.model(X=X, LV=NULL))
            strsplit(noms, ":")->noms
            ...
            paste(noms, "=~", O1[1])->model
            for(j in 2 :(length(O1))){paste(model, "+", O1[j])->model}
            model2[[i]]<-model
            model2[[1]]->model
            if(i>1) {
              for(j in 2 : i){paste(model,"\n", model2[[j]])->model   }
            }
            semPlot.model<-semPlotModel_lavaanModel(model)
            semPaths(semPlot.model, edge.label.cex = 0.65,edge.color="black", exoVar = FALSE,exoCov =T, cex=0.5)
        }
    ...
    .ez.lavaan.options<-function(model=NULL, data=NULL, X=NULL, info=TRUE, opt.list=NULL, dial=T, imp=NULL, outlier=NULL,output=NULL){
      ...
      if(dial){ 
        ...
        options<-c("estimator [estimator=ml])", "groupes [group=NULL]", "test [test=standard]", "standard.error [se=standard]", "standardisation des variables observees [std.ov=T]", 
                   "Orthogonalite des factors.[orthogonal=FALSE]", "Lien (seulement pour estimator=MML) [link=probit]",
        ...
        options<-dlgList(c("Keep the default values", options), preselect=c("estimator [estimator=ml])","test [test=standard]", "standard.error [se=standard]"), multiple = TRUE, title="What options?")$res
        ...
        if(options=="Keep the default values") return(list(mimic="default", fixed.x="default", missing="default",information="default", zero.keep.margins="default",zero.add=c(0.5,0),
                                                                 ...
                                                                 group.equal=c("loadings", "intercepts", "means", "thresholds", "regressions", "residueals", "residueal.covariances", 
                                                                               "lv.variances" , "lv.covariances")))
      } else options<-NULL
      
      
      
      if(any(options=="estimator [estimator=ml])")|is.null(opt.list$estimator) || length(opt.list$estimator)!=1|| 
         try(opt.list$estimator %in%c("ML","GLS", "WLS", "ULS", "DWLS", "MLM","MLMV","MLMVS","MLF", "MLR", "WLSM","WLSMV", "ULSM", "ULSMV" ),silent=T)!=T){
        if(info){  writeLines("[WLS] correspond a [ADF]. Les estimators avec les extensions [M],[MV],[MVSF],[R] 
                              sont des versions robusts des estimators classiques [MV],[WLS], [DWLS], [ULS]")
        ...
        if(length(opt.list$estimator)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
        return(Results)}
      } 
      ...
      if(any(options=="test [test=standard]") || length(opt.list$test)!=1 || !opt.list$test%in% c("standard", "Satorra.Bentler", "Yuan.Bentler", "mean.var.adjusted",
        ...
        if(length(opt.list$test)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
        return(Results)}
      } 
      ...
      if(dial & opt.list$test%in%c("boot","bootstrap","Bollen.Stine") || is.null(opt.list$n.boot) & opt.list$test%in%c("boot","bootstrap","Bollen.Stine")) {
        while(is.null(opt.list$n.boot)){
          ...
          if(length(n.boot)==0) {Results<-Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
          return(Results)}
      ...
      if(any(options=="standard.error [se]") || is.null(opt.list$se) || !opt.list$se%in%se1)  {
        ...
        if(length(opt.list$se)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
        return(Results)}
      } 
      ...
      if(any(options=="groupes [group=NULL]") || !is.null(opt.list$group)){
        ...
        if(is.null(group)){
          Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
          return(Results)
        } 
        group$data->data
        group$X->opt.list$group
        if(dial|| any(opt.list$group.equal %in% c("loadings", "intercepts","means","thresholds","regressions","residueals","residueal.covariances","lv.variances", "lv.covariances"))==FALSE){
          if(info) writeLines("What are the parameters that you want to keep constant?")
          opt.list$group.equal<-dlgList(c("loadings", "intercepts","means","thresholds","regressions","residueals","residueal.covariances","lv.variances", "lv.covariances"), multiple = T, 
                                        preselect=c("loadings", "intercepts","means","thresholds","regressions","residueals","residueal.covariances","lv.variances", "lv.covariances"), title="Constant parameters?")$res
          if(length(opt.list$group.equal)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
          return(Results)
          }}
        # ecrase group equal puisque aa libere les group sur cette contraintes ==> utilite ? 
        #group.partial<-dlgList(c("loadings", "intercepts","means","thresholds","regressions","residueals","residueal.covariances","lv.variances", "lv.covariances"))
        if(info) writeLines("Are the frequencies of the different groups a free parameter? ") 
        opt.list$group.w.free<-dlgList(c(TRUE, FALSE), multiple=F, preselect=FALSE, title="Constant frequency?")$res
        if(length(opt.list$group.w.free)==0) {Results<.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
          return(Results)
        }
      }
      ### zero.keep.margins
      if(any(options=="correction de continuite [zero.keep.margins]") || is.null(opt.list$zero.keep.margins)||(!is.logical(opt.list$zero.keep.margins) & opt.list$zero.keep.margins!="default")){
        if(info) writeLines("Faut-il ajouter une value aux cellules vides pour les correlations polychorique ? Pour specifier les values,choisissez TRUE, sinon choisissez [default]")
        ...
        if(length(opt.list$zero.keep.margins)==0) {
          Results<-Results<.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
          return(Results)
        }
      } 
      ...
      if( opt.list$zero.keep.margins==TRUE){
        ...
        while(is.null(opt.list$zero.add)){
          ...
          if(length(zero.add1)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
          return(Results)}
          ...
          if(is.na(zero.add1) ||  zero.add1<0 || zero.add1>1){
            ...
            opt.list$zero.add<-NA} else{
              writeLines("Please specify the value for tables wider than 2x2")
              zero.add2<-dlgInput("array> 2x2?", 0)$res
              if(length(zero.add2)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
              return(Results)}
      ...
      if(any(options=="Vraisemblance (seulement pour estimator=ML) [likelihood=default]") & opt.list$mimic=="default" & opt.list$estimator=="ML" ||is.null(opt.list$likelihood) || length(opt.list$likelihood)!=1 || try(opt.list$likelihood%in%c("wishart","normal", "default" ),silent=T)!=T) {
        ...
        if(length(opt.list$likelihood)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
        return(Results)}
      } 
      ...
      if(any(options=="Lien (seulement pour estimator=MML) [link=probit]") & opt.list$estimator=="MML" ||length(opt.list$link)!=1 || try(opt.list$link%in%c("logit","probit" ),silent=T)!=T ){
        ...
        if(length(opt.list$link)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
        return(Results)}
      }  
      ...
      if(any(options=="information [information=default]") ||is.null(opt.list$information) || try(opt.list$information%in%c("expected","observed", "default" ),silent=T)!=T ){
        ...
        if(length(opt.list$information)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
        return(Results)}
      }  
      ...
      if(any(options=="Variables exogenes fixees [fixed.x=default]") ||length(opt.list$fixed.x)!=1 || (!is.logical(opt.list$fixed.x) & opt.list$fixed.x!="default") ){
        ...
        if(length(opt.list$fixed.x)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
        return(Results)}
      }  
      
      if(any(options=="Orthogonalite des factors.[orthogonal=FALSE]") ||length(opt.list$orthogonal)!=1 || !is.logical(opt.list$orthogonal) ){
        ...
        if(length(opt.list$orthogonal)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
        return(Results)}
      } 
      ...
      if(any(options=="standardisation des variables observees [std.ov=T]") ||length(opt.list$std.ov)!=1 || !is.logical(opt.list$std.ov) ){
        ...
        if(length(opt.list$std.ov)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
        return(Results)}
      } 
      ...
      if(any(options=="Intercept des variables observees [int.ov.free=FALSE]") ||length(opt.list$int.ov.free)!=1 || !is.logical(opt.list$int.ov.free) ){
        ...
        if(length(opt.list$int.ov.free)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
        return(Results)}
      }
      ...
      if(any(options=="Intercept des variables latentes [int.lv.free=FALSE]") ||length(opt.list$int.lv.free)!=1 || !is.logical(opt.list$int.lv.free) ){
        ...
        if(length(opt.list$int.lv.free)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
        return(Results)}
      }
      ...
      if(any(options=="Estimation des indicateurs des variables latentes [std.lv=FALSE]") ||length(opt.list$std.lv)!=1 || !is.logical(opt.list$std.lv) ){
        if(info) writeLines("If true, the residueals of the latent variables are fixed at 1, otherwise the parameters of the latent variable are estimated by fixing the first indicator at 1")
        opt.list$std.lv<-dlgList(c(TRUE, FALSE ), multiple=F, preselect=FALSE, title="Vehicle standardization?")$res
        if(length(opt.list$std.lv)==0) {Results<-.ez.lavaan.options(X=X, data=data, opt.list=opt.list)
        return(Results)}
      } 
    ...
    cfa.in<-function(model=NULL,X=NULL,LV=NULL, data=NULL, ord=NULL, outlier=NULL,imp=NULL,output=NULL, info=T, opt.list=list(), sauvegarde=F){
      ...
      if(is.null(model)){ 
        ...
        if(dial || length(outlier)>1 || outlier %in% c("Complete data", "Data without influencing value") ==FALSE){
          ...
          if(length(outlier)==0) { Results<-cfa.in()
          return(Results)}
        }
        ...
        if(dial){
          ...
          if(length(type)==0) {Results<-cfa.in()
          return(Results)}
        ...
        if(type!="continues"){ 
          if(type=="mixte") {
            ...
            if(length(ord)==0) {Results<-cfa.in()
            return(Results)}
          }else ord<-X
        }
        
        model<-Lav.model(X=X, LV=LV)
        if(is.null(model)) {
          Results<-cfa.in()
          return(Results) 
        }}else{
          model<-Lav.model(model=model)
          if(is.null(model)) {
            Results<-cfa.in()
            return(Results)
          }
      ...
      Results$opt.list<-.ez.lavaan.options(data=data, X=X, info=TRUE, opt.list=opt.list, dial=dial) 
      if(is.null( Results$opt.list)) {
        Results<-cfa.in()
        return(Results)
      }
      ...
      if(dial || class(output)!="character"|| any(!output%in% c("default", "Default outputs", "parEst", "Estimated parameters", "parSt", "Standardized parameters","Adjusted covariance matrix", "fitted.cov",
                                                                "Standardized residuees", "res.St","res.Unst","Non-standardized residuees","vcov","Estimated covariance matrix",
        ...
        output<-c( "Default outputs", "Estimated parameters", "Standardized parameters","Adjusted covariance matrix", 
                   "Standardized residuees", "Non-standardized residuees","Estimated covariance matrix","AIC", "BIC", "Suitability measures", 
    ...
    cfa.out<-function(cfa.options){
      ...
      model<-cfa.options$model
      nom.v<-strsplit(model, split="[\\|,+,='\n'~' ']+")
      ...
      fit<-try( lavaan::cfa(model, data = data, ordered=ord,estimator=estimator, test=test,
      ...
      if(any(output== "default") | any(output== "Default outputs"))  {
        ...
        Results<-"To display the results, please use summary (model.cfa)"
        summary(fit)->>fit
        if(length(output)==1) fit->>model.cfa
        }
      if(any(output== "parEst") | any(output=="Estimated parameters")) parameterEstimates(fit)->Results$"Non-standard estimated parameters"
      if(any(output== "parSt") | any(output=="Standardized parameters")) standardizedSolution(fit)->Results$"Standardized estimated parameters"
      if(any(output== "Adjusted covariance matrix") | any(output=="fitted.cov")) fitted(fit)->Results$"Adjusted covariance matrix"
      if(any(output== "Standardized residuees") | any(output=="res.St")) resid(fit, type="standardized")->Results$"Standardized residuees"
      if(any(output== "Non-standardized residuees") | any(output=="res.Unst")) resid(fit)->Results$"Non-standardized residuees"
      if(any(output== "vcov") | any(output=="Estimated covariance matrix")) vcov(fit)->Result$"Estimated covariance matrix"
      if(any(output== "AIC") ) AIC(fit)->Results$AIC
      if(any(output== "BIC") ) BIC(fit)->Results$BIC
      if(any(output== "Suitability measures") | any(output=="fitM")) fitMeasures(fit)->Results$"Adjustment measure"
      if(any(output== "Inspect the starting values") | any(output=="start"))inspect(fit, what=start)->Results$"Starting values"
      if(any(output== "Inspect the model dies") | any(output=="modmat")) inspect(fit)->Results$"Model matrices"
      if(any(output== "Inspect the representation of the model") | any(output=="modrep"))inspect(fit, what=list)->Results$"Representation of the model"
    ...
    cfa.options<-cfa.in(model=model,X=X, data=data, ord=ord, outlier=outlier,imp=imp,output=output, info=T, opt.list=opt.list, sauvegarde=sauvegarde)
    if(is.null(cfa.options)) return(analyse())
    AFC<-cfa.out(cfa.options)
    if(!is.null(AFC)) Results$AFC<-AFC
    if(AFC[[1]]=="Default ouput") print(summary(model.cfa))
   ...
   try({
    ...
    call<-paste0("ez.cfa(model='", cfa.options$model, "',data=", cfa.options$nom, ",ord=", ifelse(is.null(cfa.options$ord), "NULL",paste0("c('",ord,"')")),",outlier='", outlier, 
    ...
    .add.history(data=cfa.options$data, command=Results$Call, nom=cfa.options$nom)
    .add.result(Results=Results, name =paste("AFC", Sys.time() ))  
   }, silent=T) 
    
   
    
    if(sauvegarde) save(Results=Results, choice="AFC", env=.e)
    Results$ref<-ref1(packages)
    try(ez.html(Results), silent=T)
    return(Results)
  }
