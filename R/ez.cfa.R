ez.cfa <-
  function(modele=NULL, X=NULL, data=NULL,ord=NULL, outlier="Donnees completes",imp="rm", output="default", info=T, sauvegarde=F, mimic=NULL, fixed.x="default", missing="default",information="default", zero.keep.margins="default",zero.add=c(0.5,0),
    ...
    cfa.in<-function(modele=NULL,X=NULL,LV=NULL, data=NULL, ord=NULL, outlier=NULL,imp=NULL,output=NULL, info=T, opt.list=list(), sauvegarde=F){
      ...
      if(any(is.na(data[,X]))) {
        if(is.null(imp))  {msgBox("Missing values have been detected. How do you want to treat them?")
          imp<- dlgList(c("Do nothing - Keep all observations", "Removing cases with missing values","Remplacer par la mediane","Multiple imputation - Amelia"), 
      ...
      if(dial || class(output)!="character"|| any(!output%in% c("default", "Sorties par defaut", "parEst", "Parametres estimes", "parSt", "Parametres standardises","Matrice de covariance ajustee", "fitted.cov",
        ...
        output<-c( "Sorties par defaut", "Parametres estimes", "Parametres standardises","Matrice de covariance ajustee", 
                   ...
                   "Inspecter les valeurs de depart",  "Inspecter les matrices du modele", "Inspect the representation of the model")
    ...
    cfa.out<-function(cfa.options){
      ...
      if(any(output== "parEst") | any(output=="Parametres estimes")) parameterEstimates(fit)->Resultats$"Non-standard estimated parameters"
      if(any(output== "parSt") | any(output=="Parametres standardises")) standardizedSolution(fit)->Resultats$"Standardized estimated parameters"
      ...
      if(any(output== "Inspect the representation of the model") | any(output=="modrep"))inspect(fit, what=list)->Resultats$"Representation du modele"
