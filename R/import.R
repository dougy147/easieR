import <-
  function(file=NULL, dir=NULL, type=NULL, header=T, info=TRUE, dec=".", sep=";",na.strings="NA",sheet=NULL, name="dataset"){
    ...
    if(!is.null(type)){
      ...
      if (type=="SPSS")  txt_spss_file -> type
    }
    ...
    if(any( grepl("[^[:alnum:][:space:]_.]", names(data1)))) {
      writeLines(desc_avoid_spaces_and_punctuations)
    ...
    if(any(is.na(data1))){
      writeLines(desc_number_of_missing_values)
    ...
    str(data1)
    Resultats <- desc_succesfully_imported
