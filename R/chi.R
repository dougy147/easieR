chi <-
  function(X=NULL, Y=NULL, Effectifs=NULL, p=NULL, choix=NULL, data=NULL, info=TRUE, n.boot=NULL, priorConcentration =1,  
           SampleType=NULL,fixedMargin=NULL, choix2=c("test non parametrique","Robust testing - involving bootstraps", "Facteurs bayesiens") ,rscale=2^0.5/2, html=T){
      ...
      if(choix!="Ajustement") {
        ...
        if(any(apply(comb, 1, function(x) prod(sapply(data[,x],nlevels)))>tot)){
          ...
          for(i in length(trop):1){
            msg6<-paste0("The numbers are insufficient for the number of combinations between the variable ", comb[trop[i],1], " et la variable ", comb[trop[i],2], ". This analysis will not be performed.")
            msgBox(msg6)
    ...
    chi.out<-function(data=NULL, X=NULL, Y=NULL, p=NULL, choix=NULL, Effectifs=NULL, n.boot=NULL, SampleType=NULL,
      ...
      if((choix!="Ajustement")){
        ...
        if(choix=="Independance"){
          ...
          if(any(choix2 %in% c("Test non parametrique","Robust testing - involving bootstraps")))    {
