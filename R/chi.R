chi <-
  function(X=NULL, Y=NULL, Effectifs=NULL, p=NULL, choix=NULL, data=NULL, info=TRUE, n.boot=NULL, priorConcentration =1,  
           SampleType=NULL,fixedMargin=NULL, choix2=c(TXT_non_param_test,TXT_robusts_tests_with_bootstraps, TXT_bayesian_factors) ,rscale=2^0.5/2, html=T){
    ...
    chi.in<-function(X=NULL, Y=NULL, Effectifs=NULL, p=NULL, choix=NULL, data=NULL, info=T, n.boot=NULL, SampleType=NULL, FM=NULL, choix2=NULL){
      ...
      if(choix!=TXT_chi_adjustement){
        ...
        if(choix==TXT_mcnemar_test & any(sapply(data[,c(X,Y)],nlevels)!=2)) {
          msgBox(INFO_mcnemar_need_2x2_table_yours_are_different)
      ...
      if(choix!=TXT_chi_adjustement) {
        ...
        if(any(apply(comb, 1, function(x) prod(sapply(data[,x],nlevels)))>tot)){
          ...
          for(i in length(trop):1){
            msg6<-paste0(INFO_insufficient_sample_for_combinations_between, comb[trop[i],1], INFO_and_variable_y, comb[trop[i],2], INFO_this_analysis_will_not_be_performed)
            msgBox(msg6)
            comb[ -which(dimnames(comb)[[1]]==names(trop)[i]),]->comb
          }
          if(length(comb[,1])==0) {
            msgBox(INFO_no_analysis_can_be_performed_given_your_data)
            return(NULL)
          } 
        }
      }
      ...
      if(choix==TXT_chi_adjustement) {
        ...
        while(is.null(p)){
          ...
          dlgForm(setNames(as.list(rep(1/nlevels(data[,X]),times=nlevels(data[,X]))), levels(data[,X])), INFO_probabilities_vector_please_no_fraction)$res->niveaux
          stack(niveaux)[,1]->p
      ...
      msg.options<-INFO_in_that_case_non_parametric_is_classical_chi_squared
      .ez.options(options=TXT_choice, n.boot=n.boot,param=F, non.param=T, robust=robust, Bayes=Bayes, msg.options1=NULL, msg.options2=msg.options, info=T, dial=dial, choix=choix2)->Options
      ...
      if(dial==T || any(SampleType %in% c("poisson", "jointMulti","hypergeom", "indepMulti"))==F || SampleType=="indepMulti" & any(fixedMargin %in% c("rows","cols"))==F){
        ...
        if(any(Options$choix==TXT_bayesian_factors) && choix== TXT_chi_independance ){
          ...
          for(i in 1:length(comb[,1])){
            ...
            SampleType1<-dlgList(possible, preselect=TXT_total_sample_not_fixed, multiple = FALSE, title=paste(TXT_experimental_pan_between, comb[i,1], INFO_and,comb[i,2], "?"))$res
      ...
      if((choix!=TXT_chi_adjustement)){
        ...
        if(choix==TXT_chi_independance){
          ...
          if(all(dim(tab)>2) |any(mon.chi$observed==0)) {
            INFO_odd_ratio_cannot_be_computed->Resultats$TXT_odd_ratio
          }else{
    ...
    if(length(X)>1) Resultats$TXT_alpha_warning<-paste(INFO_alpha_increased_with_value_equals_to, 100*(1-0.95^length(X)), "%", sep=" ")  
    for(i in 1:length(X)) {
      as.character(X[i])->Xi
      as.character(Y[i])->Yi
      ...
      if(chi.options$analyse==TXT_chi_adjustement) nom<-paste(INFO_chi_squared_adjustment_on_variable_x, X, sep =" ")
      if(chi.options$analyse==TXT_chi_independance) nom<-paste(TXT_chi_results_between_var_x, Xi,
                                                         INFO_and_variabe, Yi,sep=" ")
