stat.desc <-
  function(X=NULL, groupes=NULL, data=NULL, tr=.1, type=3, plot=T, ref=T, save=F, html=T){
    # X = vecteur de variables dont il faut faire les descriptive statistics (character)
    # groups [group = NULL] vecteur de variables dont il faut decomposer les descriptive statistics par sous-groupe (character)
    # data = nom de la base de donnees
    # tr = troncature
    # type = type d'asymetrie et d'aplatissement a calculer (voir ?psych::describe)
    # plot = logical. Show the plot ? 
    # ref = logical. Show packages used in this function ?
    # save = logical. Should the output be saved in rtf and R file ? 
    stat.desc.in<-function(x=NULL, groupes=NULL, data=NULL, tr=.1, type=3, save=NULL){
      list()->Results
      choice.data(data=data, info=TRUE, nom=T)->data 
      if(length(data)==0) { return(NULL)} else {
        data[[1]]->nom1
        data[[2]]->data}
      # choice X
      if(!is.null(x)) dial<-F else dial<-T
      
      msg1<-"please choose the variables for which you want to obtain descriptive statistics"
      .var.type(X=X, info=T, data=data, type=NULL, message=msg1,multiple=T, title="Variable to analyze?")->X1
      if(is.null(X1)) return(NULL)
      X1$X->x
      setdiff(names(data), x)->diff
      if(length(diff)==0 & !is.null(groupes)) {
        msgBox("Vous ne pouvez pas avoir de variable *groupes* etant donne que toutes les variables doivent etre decrites")
        groupes<-NULL
      } 
      
      if(length(diff)>0){
        if(dial){
          writeLines("You can decompose the descriptive statistics by subgroup by choosing one or more categorical variables. Do you want to specify the subgroups?")
          groupes<-dlgList(c("Yes", "non"), multiple = F, preselect="non", title="Specify groups?")$res
          if(length(groupes)==0) {stat.desc.in(x=X, groupes=NULL, data=NULL, tr=tr, type=type,save=save)->Results
            return(Results)}
          if(groupes=="non") groupes<-NULL
        }
        
        if(!is.null(groupes)){
          msg2<-"Please choose the variable (s) defining the groups"
          .var.type(X=groupes, info=T, data=data, type="factor", message=msg2,multiple=T, title="Variable (s)  groupes ?", out=x)->groupes
          if(is.null(groupes)){
            stat.desc.in(x=X, groupes=NULL, data=NULL, tr=tr, type=type,save=save)->Results
            return(Results)
          } 
          groupes$data->data
          groupes$X->groupes
        }
      } 
      
      if(dial==T | tr>1 | tr<0 | (type %in% 1:3==F) ) {
        writeLines("You can specify the truncation and parameters for flattening and skewness by choosing other options")
        options<-dlgList(c("Yes", "non"), multiple = F, preselect="non", title="Specify other options?")$res
        if(length(options)==0) {
          stat.desc.in(x=X, groupes=NULL, data=NULL, tr=tr, type=type,save=save)->Results
          return(Results)
        }
        if(options=="Yes") {opts2<-NA
        while(any(is.na(opts2))){
          dlgForm(list("Truncation: NUM"=0.1, "Type of skew and kurtosis, must be between 1 and 3: NUM"=3),  "Please set the truncation threshold")$res->opts2
          if(opts2[[1]]>0.5 | opts2[[1]]<0 ) NA->opts2[[1]] else tr<-opts2[[1]]
          if(opts2[[2]]%in% 1:3)  type<-opts2[[2]]  else opts2[[2]]<-NA  
          
        }
        }
        #      ez.options(save=T)$sauvegarde->save
      }
      Results$data<-data
      Results$nom1<-nom1
      Results$X<-x
      Results$groupes<-groupes
      Results$tr<-tr
      Results$type<-type
      Results$sauvegarde<-save
      return(Results)
    }
    
    
    
    options (warn=-1)
    c( "ggplot2", "psych", "svDialogs")->packages
    lapply(packages, require, character.only=T)
    list()->Results
    .e <- environment()
    try( windows(record=T), silent=T)->win
    if(class(win)=="try-error") quartz()
    if(!is.null(data) & class(data)!="character") deparse(substitute(data))->data
    
    stat.desc.in(x=X, groupes=groupes, data=data, tr=tr, type=type,save=save)->data.in
    if(is.null(data.in)) return(analyse())
    .stat.desc.out(X=data.in$X, groupes=data.in$groupes,data=data.in$data,plot=plot, tr=data.in$tr, type=data.in$type)->Results 
    paste(data.in$X, collapse="','", sep="")->X
    if(is.null(data.in$groupes)) paste0("'), groups [group = NULL] NULL, data=")->groupes else { paste(data.in$groupes, collapse="','", sep="")->groupes
      paste0("'), groups [group = NULL]c('",groupes ,"'), data=")->groupes}
    paste0("stat.desc(X=c('", X, groupes, data.in$nom1, ",tr=" , tr, ",type=", type, ", plot=", plot, ", ref=", ref,", html=",html, ")")->Results$Call
    .add.history(data=data.in$data, command=Results$Call, nom=data.in$nom1)
    .add.result(Results=Results, name =paste("descriptive statistics", Sys.time() ))
    
    if(data.in$sauvegarde==TRUE) save(Results=Results ,choice =paste("Descriptive statistics on",data.in$nom1 ), env=.e)
    if(ref) ref1(packages)->Results$"References of the packages used for this analysis"
    if(html) try(ez.html(Results), silent=T)
    return(Results)
  }
