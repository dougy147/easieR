  ...
  function(data=NULL, modele=NULL, Y=NULL, X_a=NULL, X_i=NULL, outlier=NULL, inf=T, select.m="none", step=NULL, group=NULL, scale=T, dial=T, info=T,
    ...
    reg.log.in<-function(data=NULL, modele=NULL, Y=NULL, X_a=NULL, X_i=NULL, outlier=NULL, inf=T, select.m="none", step=NULL, group=NULL, scale=T, dial=T, info=T,
      ...
      if(any(link %in% c("Effets additifs", "Effets d'interaction"))){
        ...
        if(length(unique(data[,Y]))!=2) {
          msg1<-paste("Votre veriable dependante a", length(unique(data[,Y])), "terms and conditions. It is incompatible with a logistic regression. She must be dichotomous" )
          msgBox(msg1)
    ...
    if(any(outlier=="Identification of influential values")|any(outlier=="Data without influencing value")|inf==T){
      ...
      if(any(outlier== "Identification of influential values")){
        ...
        data.frame("N.retirees"=N_retire, "Pourcentage.obs.retirees"=Pourcentage_retire)->Resultats$"Summary of the number of observations considered to be influential"
