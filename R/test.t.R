test.t <-
  function(X=NULL, Y=NULL, group=NULL, choice=NULL,
    ...
    test.t.in<-function(X=NULL, Y=NULL, data=NULL, choice=NULL, param=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,alternative="two.sided", 
      ...
      if(is.null(formula)){
        if(choice=="Two matched samples"){
          if(dial){
            ...
            if(length(format)==0) {
              Results<-test.t.in(X=NULL, Y=NULL, data=NULL, choice=NULL, param=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,alternative="two.sided", 
        ...
        if(length(X)>1){
          ...
          if(is.null(X)) {
            test.t.in(X=NULL, Y=NULL, data=NULL, choice=NULL, param=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,alternative="two.sided", 
      ...
      if(choice=="Comparison to a standard"){
        ...
        while(is.na(mu)){
          ...
          if(length(mu)==0) { test.t.in(X=NULL, Y=NULL, data=NULL, choice=NULL, param=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,alternative="two.sided", 
                                        formula=NULL,n.boot=NULL, rscale=NULL)->Results
            return(Results)}
          strsplit(mu, ":")->mu
          tail(mu[[1]],n=1)->mu
        ...
        if(dial){
          ...
          if(length(group)==0) { test.t.in(X=NULL, Y=NULL, data=NULL, choice=NULL, param=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,alternative="two.sided", 
                                           formula=NULL,n.boot=NULL, rscale=NULL)->Results
            return(Results)}
          data<-group$data
          group<-group$X 
      ...
      options<-.ez.options(options=c("choice","outlier"), n.boot=n.boot,param=T, non.param=T, robust=T, Bayes=T, msg.options1=msg.options1, msg.options2=msg.options2, info=info, dial=dial, 
                           choice=param, backup =sauvegarde, outlier=outlier, rscale=rscale)
      if(is.null(options)){
        test.t.in(X=NULL, Y=NULL, data=NULL, choice=NULL, param=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,alternative="two.sided", 
                  formula=NULL,n.boot=NULL, rscale=NULL)->Results
        return(Results)
    ...
    norme<-function(X, mu, data, param=c("param", "non param", "robusts"), group=NULL, alternative="two.sided", n.boot=NULL, rscale=0.707){
      ...
      if(any(param=="robusts"| any(param=="Robust testing - involving bootstraps"))){
        ...
        if(m.tr!="try-error"){
          ...
          dimnames(IC.robusts)[[1]][length(IC.robusts[,1])]<-"M-estimator modifies"}
    ...
    test.t.options<-test.t.in(X=X, Y=Y, data=data, choice=choice, param=param, outlier=outlier, backup =sauvegarde, info=info, group=group,alternative=alternative, 
    ...
    for(i in 1 : length(X)) {
      ...
      if(any(outlier=="Identification of influential values")|any(outlier=="Data without influencing value")){
        ...
        values.influentes(X="residue", critere=critere,z=z, data=data1)->influentes
      }
    ...
    Results$Call<-paste0("test.t(X=c('", X,
                           ...
                           ", choice = '", choice, 
                           "', save = ", sauvegarde, ",outlier=c('", outlier, "'),z=", ifelse(!is.null(z),z, "NULL"),
