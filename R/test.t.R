test.t <-
  function(X=NULL, Y=NULL, group=NULL, choix=NULL,
           ...
           param=c("test parametrique", "test non parametrique","Robust testing - involving bootstraps",
    ...
    # param : character vector with one or several choices among c("test parametrique", "test non parametrique","Robust testing - involving bootstraps", "Facteurs bayesiens")
    ...
    for(i in 1 : length(X)) {
      ...
      if(any(outlier== "Data without influencing value")) {
        if(length(influentes$"observations influentes")!=0 | all(outlier!="Donnees completes")){
          ...
          switch(choix,  "Comparaison a une norme"=  R1$"Data without influencing value"<-norme(X=X1, mu=mu, data=nettoyees, param=param, group=group, alternative=alternative, n.boot=n.boot, rscale=rscale),
                 "Deux echantillons apparies"=R1$"Data without influencing value"<-apparies(X=X1, Y=Y, data=nettoyees, param=param,alternative=alternative, n.boot=n.boot, rscale=rscale),
                 "Deux echantillons independants"= R1$"Data without influencing value"<-indpdts(X=X1, Y=Y, data=nettoyees, param=param,alternative=alternative, n.boot=n.boot, rscale=rscale))
        }
      }
      Resultats[[i]]<-R1
    }
