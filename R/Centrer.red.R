CBetweenr.red <-
  function(x, data=NULL, info=TRUE){options (warn=-1) 
    ...
    dlgList(c("center", "center reduce", "lower probability", "higher probability"), preselect="center reduce", multiple = TRUE, title="What do you want to do ?")$res->choice
    if(length(choice)==0) return(preprocess())
    
    for(i in 1:length(choice)){
      if(choice[i]=="center") {S<-FALSE 
