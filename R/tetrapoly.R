tetrapoly <-
  function(data=NULL,X=NULL, backup =F, ord=NULL ,info=T, group=NULL, estimator="two.step", output="cor", imp=NULL, html=T){
    # data : dataframe
    ...
    if(is.null(X)) {
      Results<-tetrapoly(data=NULL,X=NULL, backup =F, ord=NULL ,info=T, group=NULL, estimator=estimator, output=output)
      return(Results)}
    data<-X$data
    X<-X$X
    ...
    Results$Call<-paste0("tetrapoly(data=", nom,",X=c('", X,"'), backup =", sauvegarde, ",ord=", ifelse(!is.null(ord),paste0("c('",ord,"')"), "NULL"),
