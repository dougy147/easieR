factor.an <-
  function(data=NULL, X=NULL, nF=NULL, rotation="none", methode="ml", sat=0.3, outlier=c("Donnees completes"),
    ...
    # outlier : one among "Donnees completes" or "Data without influencing value"
    ...
    fa.in<-function(data=NULL, choix=NULL, X=NULL, imp=NULL, ord=NULL, nF=NULL, rotation="none", methode="minres", sat=NULL, 
      ...
      if(choix=="fa" | choix=="Analyse factorielle exploratoire") msg3<-"Please choose the variables you want to analyze. You must choose at least 5 variables" else{
      ...
      if(is.na(KMO1)) {
        ...
        Resultats$"Mesure d'adequation de la matrice"$"Global Kaiser-Meyer-Olkin Index"<-"The KMO could not be calculated. Check your correlation matrix."
      } else {
        round(KMO1$MSA,3)->Resultats$"Mesure d'adequation de la matrice"$"Global Kaiser-Meyer-Olkin Index" ### doit etre superieur a 0.5 sinon la matrice ne convient pas pour analyse factorielle. Dans lÃÂÃÂideal, avoir au moins 0.8. Si des X presentent un KMO<0.5, on peut envisager de les supprimer. 
        round(KMO1$MSAi,3)->Resultats$"Mesure d'adequation de la matrice"$'Indice de Kaiser-Meyer-Olkin par item'
        round(det(Matrice),5)->Resultats$"Mesure d'adequation de la matrice"$"Determinant of the correlation matrix"
        Resultats$"Mesure d'adequation de la matrice"$"Determinant of the correlation matrix: information"<-"risk of multicollinearity if the determinant of the matrix is less than 0.00001"
      }
      ...
      if(dial){
        ...
        line <- readline()  
        dlgMessage(c("Is the matrix satisfactory for an AFE?", "Continuer ?"), "okcancel")$res->res.kmo
      ...
      if(dial | is.null(nF) | !is.numeric(nF)) {
        msgBox(paste("the number of factors to be retained according to the parallel analysis is",result$Components$nparallel, "facteurs." ))
        cat ("Appuyez sur [entree] pour continuer")
        line <- readline() 
        nF<-NA
        while(!is.numeric(nF)) {
    ...
    fa.out<-function(Matrice, data, X, nF, methode, rotation, sat, scor.fac, n.boot, nom, hier=FALSE, cor="cor", nfact2){
      ...
      if(nF>1){dimnames(var.ex)[[1]]<-c("Sommes des carres des saturations", "proportion of variance explained",
                                        "cumulative explained variance proportion", "Proportion de l'explication", 
                                        "Cumulative proportion of the explanation")} else {
                                          dimnames(var.ex)[[1]]<-c("Sommes des carres des saturations", "proportion of variance explained")
                                        }
      ...
      if(length(X)>5){
        ...
        c("chi square of the null model", "Degrees of freedom of the null model", "objective function of the null model",
          "degrees of freedom of the model", "objective function of the model", "RMSEA", "limite inferieure du RMSEA", "limite superieure du RMSEA",
          "Confidence threshold (1- alpha)", "Tucker Lewis Reliability Factor - TLI", "BIC", "EBIC", 
          "RMSR", "RMSR corrige", "Matching based on values outside the diagonal", "chi carre empirique", "value of the proability of the empirical square footage",
          "chi-square of maximum likelihood", "value of the chi-square probability of the maximum likelihood", "nombre total d'observations")->dimnames(stats)[[1]]
      ...
      if(hier) {
        ...
        var.ex <- round(FA.results$Vaccounted,3)
        if(nfact2>1){dimnames(var.ex)[[1]]<-c("Sommes des carres des saturations", "proportion of variance explained",
                                              "cumulative explained variance proportion", "Proportion de l'explication", 
                                              "Cumulative proportion of the explanation")} else {
                                                dimnames(var.ex)[[1]]<-c("Sommes des carres des saturations", "proportion of variance explained")
                                              }
        Resultats$"Hierarchical factor analysis"$"Variance expliquee"<-var.ex
    ...
    acp.out<-function(Matrice, data, X, nF, methode, rotation, sat, scor.fac, nom){
      ...
      c("Degrees of freedom of the null model", "objective function of the null model","degrees of freedom of the model", "objective function of the model",
        "RMSR",  "Matching based on values outside the diagonal", "chi carre empirique", "value of the probability of the empirical chi-square",
        "chi-square of maximum likelihood", "value of the chi-square probability of the maximum likelihood", "nombre total d'observations")->dimnames(stats)[[1]]
