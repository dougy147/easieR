...
fiabilite <-
  function(X=NULL,Y=NULL, data=NULL, choix=NULL, ord=NULL,outlier="Donnees completes", keys=NULL, n.boot=NULL, sauvegarde=F, 
    ...
    if(dial || is.null(choix) || length(choix)!=1 ||choix %in% c("Alpha de Cronbach","alpha","ICC","CCK","Correlation intra-classe","Kendall coefficient of concordance")==FALSE){
      dial<-T  
      writeLines("Please choose the analysis you want to perform.")
      dlgList(c("Alpha de Cronbach", "Correlation intra-classe","Kendall coefficient of concordance"), preselect=NULL, multiple = FALSE, title="What analysis do you want to perform?")$res->choix
    ...
    if(choix %in% c("Alpha de Cronbach","Correlation intra-classe","ICC","alpha") ){
     ...
     writeLines("influencing values are identified based on the Mahalanobis distance with a chi threshold of 0.001")
      ...
      if(choix=="Correlation intra-classe"| choix=="ICC"){psych::ICC(data[,X], missing=FALSE)->ICC.out
        ...
        Resultats$"informations"<-paste("le nombre de juge =", length(X), "and the number of observations =", ICC.out$n.obs) } 
    }
    ...
    if(choix=="Kendall coefficient of concordance"){  
      ...
      dimnames(CK.out$confid)<-list(c("Unweighted kappa coefficient","Coefficient kappa pondere"),c("lim.inf","estimation","lim.sup"))
