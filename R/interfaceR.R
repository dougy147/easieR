interfaceR <-
  function(){
    ...
    choix <- dlgList(c("get the working directory","specify the working directory", "Deleting an object in memory", 
                       "list of objects in memory", "search for a new function", "mise a jour des packages","Check the installation of packages"), preselect=NULL, multiple = FALSE, title="What is your choice ?")$res
    while(length(choix)==0) return(easieR())
    
    switch(choix, 
           "get the working directory" = Resultats$"Repertoire de travail" <- getwd(),
           "list of objects in memory"= Resultats$"Objets en memoire" <- ls(envir=.GlobalEnv),
