ez.mediation <-
  function(info=T, html=T){
    ...
    mediation.effect.bar.plot2 <-function (x, mediator, dv, main = "Mediation Effect Bar Plot",
      ...
      if (save.pdf == TRUE) {
        if (save.eps == TRUE)
          stop(desc_only_one_file_format_at_time_EPS_PDF)
        if (save.jpg == TRUE)
          stop(desc_only_one_file_format_at_time_PDF_JPG)
      }
      if (save.eps == TRUE) {
        if (save.jpg == TRUE)
          stop(desc_only_one_file_format_at_time_EPS_JPG)
      }
    ...
    Resultats<-list()
    dlgList(c(txt_simple_mediation_effect,
              txt_distance_mediation_effect), preselect=NULL, multiple = FALSE, title=ask_mediation_type)$res->choix
    ...
    if(choix==txt_simple_mediation_effect){
      ...
      Resultats$Information<-txt_for_a_detailed_results_description_mediation
    ...
    }else { print(desc_unavailable_distal_mediations)
