corr.complet <-
  function(X=NULL, Y=NULL, Z=NULL,data=NULL,  group=NULL, param=c("test parametrique", "test non parametrique","Robust testing - involving bootstraps", "Facteurs bayesiens"), 
      ...
      if(any(param=="non param")| any(param=="Test non parametrique")){
        ...
        if(!is.null(group)){
          ...
          if(choix!="Correlations"){
            ...
            BPgroup<-data.frame(gr.l,BPgroup )
            Resultats$"Test non parametrique"$"Partial Spearman correlation by group"<-BPgroup 
