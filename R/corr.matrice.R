  ...
  function(X=NULL, Y=NULL, Z=NULL,data=NULL, group=NULL,method="pearson",param=c("H0","FB"), sauvegarde=F, outlier=c("Complete data"),n.boot=1,  rscale=0.354, info=T,
    ...
    corr.matrice.in<-function(X=NULL, Y=NULL, Z=NULL, group=NULL, data=NULL, p.adjust="holm", rscale=0.354,sauvegarde=F,outlier="Complete data", info=T, method="pearson", param=c("H0","FB"), n.boot=NULL){
      ...
      if(dial || length(outlier)>1 || outlier %in% c("Complete data", "Data without influencing value") ==FALSE){
        ...
        if(length(outlier)==0) { Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, method=NULL, param=NULL, outlier=NULL, sauvegarde=NULL, info=T, group=NULL,
      ...
      if(dial || length(method)>1 || method %in% c("pearson", "spearman","kendall") ==FALSE){
        ...
        if(length(method)==0) { Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, method=NULL, outlier=NULL, sauvegarde=NULL, info=T, group=NULL,
                                                           n.boot=NULL, rscale=0.353)
        return(Results)}
      }
      ...
      if(is.null(Y) & is.null(Z)){
        ...
        while(is.null(n.boot)){
          ...
          if(length(n.boot)==0) {Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, method=NULL, outlier=NULL, sauvegarde=NULL, info=T, group=NULL,
                                                            n.boot=NULL, rscale=0.353)
          return(Results)}
      ...
      if((dial)|| !is.null(rscale) & ((is.numeric(rscale) & (rscale<0.1 | rscale>2)) || (!is.numeric(rscale) & rscale%in% c("medium", "wide", "ultrawide")==F))) {
        ...
        if(length(param)==0) { Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, param=NULL, outlier=NULL, sauvegarde=NULL, info=T, group=NULL,
                                                          n.boot=NULL, rscale=0.353)
        return(Results)}
        ...
        if(any(param=="Bayesian factors") | any(param=="FB")){
          ...
          if(length(rscale)==0) {
            Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, param=NULL, outlier=NULL, sauvegarde=NULL, info=T, group=NULL,
                                       n.boot=NULL, rscale=0.353)
            return(Results)
          }
      ...
      if(any(param=="H0 tests") |any(param=="H0")){
        if(dial | length(p.adjust)!=1 || p.adjust %in% c("holm", "hochberg", "hommel", "bonferroni", "BH", "BY","fdr", "none")==FALSE){
          ...
          if(length(p.adjust)==0) {Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, param=NULL, outlier=NULL, sauvegarde=NULL, info=T, group=NULL,
                                                              n.boot=NULL, rscale=0.353)->Results
          return(Results)}
        } 
      } else p.adjust<-"none"
      if(dial | length(sauvegarde)!=1 || !is.logical(sauvegarde )){
        ...
        if(length(sauvegarde)==0) {Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, param=NULL, outlier=NULL, sauvegarde=NULL, info=T, group=NULL,
                                                              n.boot=NULL, rscale=0.353)->Results
        return(Results)}
    ...
    Results$"Matrix of correlations"<-corr.matrice.out(data=data, X=X, Y=Y, Z=Z, p.adjust=p.adjust, method=method,sauvegarde=sauvegarde, rscale=rscale, n.boot=n.boot, param=param)
