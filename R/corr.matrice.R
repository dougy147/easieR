corr.matrice <-
  function(X=NULL, Y=NULL, Z=NULL,data=NULL, group=NULL,method="pearson",param=c("H0","FB"), backup =F, outlier=c("Complete data"),n.boot=1,  rscale=0.354, info=T,
    ...
    # rscale : numeric. If not null, bayesian factors are computed. Can also be "medium", "wide", "ultrawide"
    ...
    corr.matrice.in<-function(X=NULL, Y=NULL, Z=NULL, group=NULL, data=NULL, p.adjust="holm", rscale=0.354, backup =F,outlier="Complete data", info=T, method="pearson", param=c("H0","FB"), n.boot=NULL){
      Results<-list()
      ...
      if(is.null(choice) ){
        ...
        choice<-dlgList(c("Correlations", "Partial correlations"), preselect="Correlations", multiple = FALSE, title="Correlations or partial correlations?")$res
      ...
      if(choice=="Correlations" & dial==T){
        writeLines("Une matrice carree est une matrice avec toutes les Correlations 2 a 2. 
                   Une matrice rectangular est une matrice dans laquelle un premier ensemble de variables est mis en correlations avec un second jeu de variables")
        carre<-dlgList(c("carree", "rectangular"), multiple = FALSE, title="die type")$res
        if(length(carre)==0){Results<-corr.matrice.in()
        return(Results)}
      } else carre<-"carree"
      ...
      if(is.null(X)) {
        corr.matrice.in(X=NULL, Y=NULL, data=NULL, method=NULL, param=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,
                        n.boot=NULL, rscale=0.353)->Results
        return(Results)}
      data<-X$data
      X1<-X$X
      ...
      if(choice=="Partial correlations"){
      ...
      if(length(group)==0) {   corr.matrice.in(X=NULL, Y=NULL, data=NULL, method=NULL, param=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,
                                               n.boot=NULL, rscale=0.353)->Results
        return(Results)}
      data<-group$data
      group<-group$X 
      ...
      if(dial || length(outlier)>1 || outlier %in% c("Complete data", "Data without influencing value") ==FALSE){
        ...
        if(length(outlier)==0) { Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, method=NULL, param=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,
      ...
      if(dial || length(method)>1 || method %in% c("pearson", "spearman","kendall") ==FALSE){
        ...
        if(length(method)==0) { Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, method=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,
                                                           n.boot=NULL, rscale=0.353)
        return(Results)}
      }
      ...
      if(is.null(Y) & is.null(Z)){
        ...
        while(is.null(n.boot)){
          ...
          if(length(n.boot)==0) {Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, method=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,
                                                            n.boot=NULL, rscale=0.353)
          return(Results)}
      ...
      if((dial)|| !is.null(rscale) & ((is.numeric(rscale) & (rscale<0.1 | rscale>2)) || (!is.numeric(rscale) & rscale%in% c("medium", "wide", "ultrawide")==F))) {
        if(info) writeLines("Voulez-vous les tests d'hypothees nuls ou/et les factors.bayesiens ?")   
        ...
        if(length(param)==0) { Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, param=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,
                                                          n.boot=NULL, rscale=0.353)
        return(Results)}
        ...
        if(any(param=="Bayesian factors") | any(param=="FB")){
          ...
          if(length(rscale)==0) {
            Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, param=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,
                                       n.boot=NULL, rscale=0.353)
            return(Results)
          }
          ifelse(rscale=="medium", rscale<-2^0.5/4, ifelse(rscale=="wide", rscale<-0.5, ifelse(rscale=="ultrawide", rscale<-2^0.5/2, rscale<-rscale)))} else rscale<-NULL
      } 
      ...
      if(any(param=="H0 tests") |any(param=="H0")){
        if(dial | length(p.adjust)!=1 || p.adjust %in% c("holm", "hochberg", "hommel", "bonferroni", "BH", "BY","fdr", "none")==FALSE){
          ...
          if(length(p.adjust)==0) {Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, param=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,
                                                              n.boot=NULL, rscale=0.353)->Results
          return(Results)}
        } 
      } else p.adjust<-"none"
      if(dial | length(sauvegarde)!=1 || !is.logical(sauvegarde )){
        ...
        if(length(sauvegarde)==0) {Results<-corr.matrice.in(X=NULL, Y=NULL, data=NULL, param=NULL, outlier=NULL, backup =NULL, info=T, group=NULL,
                                                              n.boot=NULL, rscale=0.353)->Results
        return(Results)}
    ...
    corr.matrice.out<-function(data, X, Y, Z, p.adjust, method,sauvegarde, rscale, n.boot, param){
      Results<-list()
      Results$"Descriptive statistics"<-.stat.desc.out(X=c(X,Y,Z), groupes=NULL, data=data, tr=.1, type=3, plot=F)
      Results$"Multivariate normalcy"<-.normalite(data, c(X,Y,Z)) 
      ...
      if(is.null(Z)){
        ...
        Results$"Correlation matrix"<-as.data.frame(r1)
        ...
        Results$"Partial Correlations Matrix" <-as.data.frame(r1)
      }    
      
      class(r1)<-"matrix"
      dimnames(r1)[[1]]<-paste(dimnames(r1)[[1]], "r")
      matrice$n->Results$"sample size"
      
      if(any(param=="H0")|any(param=="H0 tests")) {paste("the correction applied is the correction of",p.adjust)->Results$Correction[1]
        if(is.null(Y)) Results$Correction[2]<-"Only values above the diagonals are adjusted for multiple comparisons"
        round(matrice$p,3)->r2
        class(r2)<-c("matrix", "p.value")
        Results$"probability matrix"<-r2
      ...
      if(method=="kendall") {
        ...
        Results$"Information"<-"The effect size is calculated from the formula proposed by Walker, 2003"   
      ...
      if(!is.null(rscale)){
        ...
        Results$"Bayesian factors"<-as.data.frame(r3)
        r1<-rbind(r1, r3)
      }
      class(r2)<-"matrix"
      if(is.null(Y)) r2[which(lower.tri(r2, diag = T))]<-"-"
      Results$"r. two matrix" <-as.data.frame(r2)
      ...
      nice.mat<-list()
      nice.mat$"Correlation matrices"<-(r1)
      if(html) try(ez.html(nice.mat), silent =T)

      
      if(is.null(Y) & is.null(Z) & (!is.null(n.boot) && n.boot > 100)) round(cor.ci(data[,X], n.iter=n.boot, plot=FALSE)$ci,4)->Results$"Confidence interval estimated by bootstrap" else  round(matrice$ci,4)->Results$"Confidence interval" 
      names(Results[[length(Results)]])<-c("inf.lim","r","sup.lim","p-value")
    ...
    corr.options<-corr.matrice.in(X=X, Y=Y, Z=Z, data=data, group=group, param=param, outlier=outlier, backup =sauvegarde, info=T,  rscale=rscale, n.boot=n.boot)
    ...
    Results$"Matrix of correlations"<-corr.matrice.out(data=data, X=X, Y=Y, Z=Z, p.adjust=p.adjust, method=method, backup =sauvegarde, rscale=rscale, n.boot=n.boot, param=param)
    ...
    if(!is.null(group))   {
      ...
      for(i in 1:length(G)){
        resg<-corr.matrice.out(data=G[[i]], X=X, Y=Y, Z=Z, p.adjust=p.adjust, method=method, backup =sauvegarde, rscale=rscale, n.boot=n.boot, param=param)  
        Results[[length(Results)+1]]<-resg
        names(Results)[length(Results)]<-names(G)[i]
      }
    } 
    ...
    Results$Call<-paste0("corr.matrice(X=c('", X,
    ...
    if(sauvegarde) save(Results=Results, choice=paste("correlation of", method), env=.e)
