maths <-
  function(info=TRUE){
    ...
    dlgList(c("additions","multiplication", "division", "soustraction","moyenne de colonnes", "exposant ou racine", 
              "logarithme", "exponentiel","valeur absolue","modele complexe"), preselect="additions", multiple = FALSE, title="Which operation do you want?")$res->choix
    ...
    if(choix=="additions") {
      ...
      dlgList(c("addition de colonnes","addition of a specific value"), preselect="addition de colonnes", multiple = TRUE, title="Which operation do you want?")$res->choix2
    ...
    if(choix=="multiplication"){
      ...
      dlgList(c("multiplication de colonnes","multiplication of a specific value"), preselect="multiplication de colonnes", multiple = TRUE, title="Which operation do you want?")$res->choix2
      if(length(choix2)==0) return(maths())
    ...
    if(choix== "logarithme"){
      if(info=="TRUE") writeLines("Please select the variables to be logarithmed ")
      variable(multiple=TRUE)->X
      if(length(X)==0|| any(X=="annuler")) return(maths())
      ...
      assign(nom1, data, envir=.GlobalEnv)
      paste("le logarithme de base", Y, " has been applied to the variable", X)->Resultats
    }
