maths <-
  function(info=TRUE){
    ...
    dlgList(c("additions","multiplication", "division", "soustraction","average of columns", "exponent or root", 
              "logarithm", "exponential","absolute value","complex model"), preselect="additions", multiple = FALSE, title="Which operation do you want?")$res->choix
    ...
    valeur<-function(info=TRUE, out=NULL){
      ...
      msg<-"no"
      while(msg=="no" ){
        valeur1 <- dlgInput("What value do you want for your math operation?", out)$res 
        if(length(valeur1)!=0){
        ...
        if(is.na(valeur1) ) { dlgMessage("the value you entered is not numeric. Do you want to cancel this scan?", "yesno")$res->msg
    ...
    nom<-function(data,info, nom1){
      ...
      Resultats<-paste("The variable", variable, "has been added to", nom1)
      return(Resultats)}
    ...
    if(choix=="additions") {
      ...
      dlgList(c("addition of columns","addition of a specific value"), preselect="addition of columns", multiple = TRUE, title="Which operation do you want?")$res->choix2
      if(length(choix2)==0) return(maths())
      if(any(choix2== "addition of a specific value")){
        variable()->X
        ...
        if(valeur1>0)      names(data)[(length(data)-(length(X)-1)):length(data)]<-paste(X, "plus", valeur1, sep=".") else names(data)[(length(data)-(length(X)-1)):length(data)]<-paste(X, "less", abs(valeur1), sep=".")
        assign(nom1, data, envir=.GlobalEnv)
        paste(valeur1, "has been added to the variable", X)->Resultats
      }
      
      if(any(choix2== "addition of columns")) {
    ...
    if(choix=="multiplication"){
      ...
      dlgList(c("multiplication of columns","multiplication of a specific value"), preselect="multiplication of columns", multiple = TRUE, title="Which operation do you want?")$res->choix2
      if(length(choix2)==0) return(maths())
      if(any(choix2== "multiplication of a specific value")){
        ...
        if(is.null(valeur1)) return(maths())
        data.frame(data, data[,X]*valeur1)->data
        names(data)[(length(data)-(length(X)-1)):length(data)]<-paste(X, "multiplied by", valeur1, sep=".")
        assign(nom1, data, envir=.GlobalEnv)
        paste(valeur1, "a multiplies the -variable-s", X)->Resultats
      }
      
      if(any(choix2== "multiplication of columns")) {
        variable()->X
        if(length(X)==0|| any(X=="to cancel")) return(maths())
    ...
    if(choix=="division"){
      ...
      if(numer=="value") valeur(info=info, out=1)->X else{
      ...
      if(denom=="value") valeur(info=info, out=1)->Y else{
    ...
    if(choix=="soustraction") {
      ...
      if(length(numer)==0) return(maths())
      if(numer=="value") valeur(info=info, out=0)->X else{
      ...
      if(length(denom)==0) return(maths())
      if(denom=="value") valeur(info=info, out=0)->Y else{
      ...
      assign(nom1, data, envir=.GlobalEnv)
      #nom(data=data, info=info,nom1=nom1)->Resultats
      Resultats<-"The mathematical operation took place correctly."
      }
    
    if(choix=="average of columns")  {
    ...
    if(choix== "exponent or root"){
      ...
      assign(nom1, data, envir=.GlobalEnv)
      paste("the variable", X, " a ete elevee a la puissance", Y)->Resultats
    ...
    if(choix== "logarithm"){
      ...
      assign(nom1, data, envir=.GlobalEnv)
      paste("the base logarithm", Y, " a ete applique a la variable", X)->Resultats
    }
    if(choix== "exponential"){
      ...
      assign(nom1, data, envir=.GlobalEnv)
      paste("the exponential has been applied to the variable", X)->Resultats
    }
    if(choix== "absolute value"){
      ...
      assign(nom1, data, envir=.GlobalEnv)
      paste("the absolute value has been applied to the variable", X)->Resultats
    }
    if(choix== "complex model"){
      ...
      print(names(data))
      valeur1 <- dlgInput("Please specify the model to be produced")$res 
